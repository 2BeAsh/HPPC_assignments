...
public:
    ...
    Water() {
        // #pragma acc parallel loop gang vector collapse(2) copy(e)  // Often causes errors, commented out
...
void exchange_horizontal_ghost_lines(grid_t& data, int qnum) {
    #pragma acc parallel loop async present(data) num_gangs(NUM_GANGS)  // OBS Skal man sige hvad størrelse data har? Fx data[1:] 
...
void integrate(Water &w, const real_t dt, const real_t dx, const real_t dy, const real_t g) {
        ...
        #pragma acc parallel loop async(1) present(w) collapse(2) num_gangs(NUM_GANGS)
