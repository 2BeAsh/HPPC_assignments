...
const long NTHREADS=1;            // number of threads  - WEAK SCALING: SCALE WITH NUMBER OF THREADS. ONLY MULTIPLE OF TWO. Når laver weak scaling, match NTHREADS og OMP
...
void fft(std::vector<Complex>& x)
{
	...
	// conquer
	fft(even);  // OBS Anvend fibonacci fra forelæsning
	fft(odd);
...
int main(int argc, char* argv[]){    
    ...
    std::cout <<  "Checksum    :" << std::setw(20) << std::setprecision(15)  // Depends on number of frequencies. 
              << checksum << "\n";
}
