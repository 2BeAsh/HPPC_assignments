...
void master (int nworker) {
    ...
    int shutdown_tag = NTASKS + 10;
    for (int j = 1; j <= nworker; j++) {
        // Get and store result
        MPI_Recv(&current_result, 1, MPI_INT, j, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        int result_index = status.MPI_TAG;
        result[result_index] = current_result;
        // Send shutdown 
        MPI_Send(&result[0], 1, MPI_INT, j, shutdown_tag, MPI_COMM_WORLD);
    }
...
void worker (int rank) {
    ...
    while (received_tag != NTASKS + 10) {  // NTASKS + 10 is shutdown tag
